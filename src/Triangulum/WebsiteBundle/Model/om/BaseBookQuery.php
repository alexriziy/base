...
use \PropelPDO;
use Triangulum\WebsiteBundle\Model\Author;
use Triangulum\WebsiteBundle\Model\Book;
use Triangulum\WebsiteBundle\Model\BookPeer;
use Triangulum\WebsiteBundle\Model\BookQuery;
...
abstract class BaseBookQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'default', $modelName = 'Triangulum\\WebsiteBundle\\Model\\Book', $modelAlias = null)
    {
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \Triangulum\WebsiteBundle\Model\AuthorQuery A secondary query class using the current class as primary query
     */
    public function useAuthorQuery($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        return $this
            ->joinAuthor($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Author', '\Triangulum\WebsiteBundle\Model\AuthorQuery');
    }
